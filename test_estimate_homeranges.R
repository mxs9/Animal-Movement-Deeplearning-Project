...
----------------------------------------------
  ...
  run_akde_analysis <- function(name1, name2, study, date_encounter){
    ...
    folder_dir <- "/Users/Qianr/OneDrive/Documents/MovementSeminar/selected_pairs"
...
---------------------------------------
  ...
  Apethe_Merimela_encounter_date <-ymd_hms("2016-02-10 20:00:00")
#Get all data on both pairs
Apethe_data <- movement_data_id_all4 %>% filter(individual.local.identifier == "Apethe", study.id == "Wilmers")
Merimela_data <- movement_data_id_all4 %>% filter(individual.local.identifier == "Merimela", study.id == "Wilmers")
...
#Split before data sets
Apethe_before <- Apethe_data %>% filter(timestamp < ymd_hms("2016-02-10 19:50:00"))
Merimela_before <- Merimela_data %>% filter(timestamp < ymd_hms("2016-02-10 19:50:00"))
#Split after data sets
Apethe_after <- Apethe_data %>% filter(timestamp > ymd_hms("2016-02-10 20:00:00"))
Merimela_after <- Merimela_data %>% filter(timestamp > ymd_hms("2016-02-10 20:00:00"))

#Make lists to input into function (MAKE SURE IT"S NAMES)
before_list <- list(Apethe = Apethe_before, Merimela = Merimela_before)
after_list <- list(Apethe = Apethe_after, Merimela = Merimela_after)
#Run Function!
run_akde_with_data(before_list, after_list, Apethe_Merimela_encounter_date)
